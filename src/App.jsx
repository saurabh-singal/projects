...
function App() {
  ...
  const handleEdit = (e, id) => {
    let curr = todos.filter((i)=>i.id===id)
    setTodo(curr[0].todo)
        ...
        <div className="todos">
          ...
          <div key={item.id} className="todo flex md:w-1/2 my-3 justify-between">
            ...
            <div className="buttons flex h-full">
              <button onClick={(e)=>handleEdit(e, item.id)} className="bg-violet-800 hover:bg-slate-950 p-2 py-1 text-sm font-bold text-white rounded-md mx-1"><FaEdit/></button>
              <button onClick={()=>handleDelete(item.id)} className="bg-violet-800 hover:bg-slate-950 p-2 py-1 text-sm font-bold text-white rounded-md mx-1"><MdDelete/></button>
            </div>
          </div>
            ))
        }
